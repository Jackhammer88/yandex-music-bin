...
pkgver=5.75.2     # текущая версия приложения
pkgrel=2
...
package() {
  ...
  if [[ -f "usr/share/applications/yandexmusic.desktop" ]]; then
    ...
    sed -E \
      -e 's|^Exec=.*|Exec=/opt/yandex-music/yandexmusic --gtk-version=3 %U|' \
...
Exec=/opt/yandex-music/yandexmusic --gtk-version=3 %U
Terminal=false
Type=Application
Icon=yandex-music
Categories=Audio;
...
EOF
  ...
  # права на chrome-sandbox: по умолчанию без SUID
